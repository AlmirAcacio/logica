programa
{
   inclua biblioteca Util   -->  U						// Inclusão da biblioteca Util no código do programa. (-->  U representa a abreviação desta biblioteca no código.)
   ...
   funcao inicio()
   {
      ...
      contagemFACA(contador)							// Instrução de chamada da função contagemFACA. Ela recebe a variável contador, que armazena a constante TAMANHO, como parâmetro.
   	
      contagemENQUANTO(contador)						// Instrução de chamada da função contagemENQUANTO. Ela também recebe a variável contador, que contém a constante TAMANHO, como parâmetro.
      
	 contagemPARA(contador)							// Instrução de chamada da função contagemPARA, que também recebe como parâmetro a variável contador, que armazena a contante TAMANHO.	
   }
   
   funcao contagemFACA(inteiro contador)		   // Definição da função contagemFACA, que possui como parâmetro um inteiro, que será a variável contador, declarada na função inicio.
   {
